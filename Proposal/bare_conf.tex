...
Since then, I learnt during office hours after class that my understanding about the perceived superiority of SVMs when compared to a Neural Network was not correct any more. Neural Networks have surpassed SVMs in image recognition. Very Deep Convolutional Networks \cite{VGG} are currently the state of the art in image recognition. The suggestion I got during office hours was to use the input to the final layer of a Visual Geometry Group (VGG) Very Deep Convolutional Net (VGG NET) classifier as features for an SVM to do linear separation. 
...
$$
\textbf{x} = \sum_{i=1}^m \alpha_i y_i \textbf{x}_i
$$
...
The INRIA labelled dataset for humans present and absent was used for training and testing. All the images were of the same size and due to this, no padding of training data vectors with zeroes needed to be done as all the pixel array vectors had the same length.
...
\item All the images were of the same size and did not need to pad the feature vectors with zeroes in order to make them of the same size.
